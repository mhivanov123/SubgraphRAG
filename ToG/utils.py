...
def prepare_dataset2(dataset_name):
    if dataset_name == 'cwq':
        ...
        with open('../retrieve/data_files/webqsp/webqsp/WebQSP.json',encoding='utf-8') as f:
            datas = json.load(f)
...
def prepare_dataset(dataset_name, pred_file_path, score_dict_path, split, prompt_mode, seed=0, triplets_to_sample=[50, 100, 200, 300]):
    
    pred_file_path = f"../reason/results/KGQA/{dataset_name}/RoG/{split}/results_gen_rule_path_RoG-{dataset_name}_RoG_{split}_predictions_3_False_jsonl/predictions.jsonl"
